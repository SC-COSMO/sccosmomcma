...
# Figure X: comparison between approaches ---------------------------------
...
# Save data
save(df_figX, file = "figs/data_frames/df_figX_Diff_SQ_SA.RData")
...
for(n_outcome in select_outcome){
  ...
  # Save plot
  ggsave(file = paste0("figs/figs_paper/figX_Diff_",abbrev_plot,"_SA_SQ.jpg"),
...
df_outcome_int <- df_out_total_prob_all %>%
  ...
  group_by(county, type, BaseCase, BaseCase_type, Intervention, 
           intervention_type, Outcome, dates, proj_type) %>%
  summarise(mean = mean(value),
  ...
  select(state, type, BaseCase, BaseCase_type, Intervention, 
         intervention_type, Outcome, dates, value, lb, ub, time_stamp, proj_type) %>%
  ungroup()
...
save(df_fig1_CasesDeaths,
     file = paste0("figs/data_frames/df_fig1_CasesDeaths.RData"))
...
df_hosp_observed <- rbind.data.frame(df_hosp_tot %>% mutate(BaseCase_type = "Holidays",
                                                            BaseCase      = v_basecase_names["Holidays"],
                                                            proj_type     = "SQ"),
                                     df_hosp_tot %>% mutate(BaseCase_type = "Holidays",
                                                            BaseCase      = v_basecase_names["Holidays"],
                                                            proj_type     = "SA"),
...
if(offset_flag){
  save(df_fig2_HospTot, 
       file = paste0("figs/data_frames/df_fig2_HospTot_offset.RData"))
  
}else{
  save(df_fig2_HospTot, 
       file = paste0("figs/data_frames/df_fig2_HospTot.RData"))
}
...
save(df_fig3_CasesDeaths_interv,
     file = paste0("figs/data_frames/df_fig3_CasesDeaths_interv.RData"))
...
# Create data.frame with two set of base-case
df_hosp_observed <- rbind.data.frame(df_hosp_fig4 %>% mutate(BaseCase_type = "Holidays",
                                                             proj_type = "SQ"),
                                     df_hosp_fig4 %>% mutate(BaseCase_type = "Holidays",
                                                             proj_type = "SA"),
                                     df_hosp_fig4 %>% mutate(BaseCase_type = "StatusQuo",
                                                             proj_type = "SQ"),
                                     df_hosp_fig4 %>% mutate(BaseCase_type = "StatusQuo",
                                                             proj_type = "SA"))

# Order base-case
v_basecase <- as.character(unique(df_hosp_observed$BaseCase_type))
df_hosp_observed$BaseCase_type <- ordered(df_hosp_observed$BaseCase_type,
                                          rev(v_basecase))
...
if(offset_flag){
  save(df_fig4_HospTot_interv,
       file = paste0("figs/data_frames/df_fig4_HospTot_interv_offset.RData"))
}else{
  save(df_fig4_HospTot_interv,
       file = paste0("figs/data_frames/df_fig4_HospTot_interv.RData"))
}
...
save(df_fig5_ExceedHospCap,
     file = paste0("figs/data_frames/df_fig5_ExceedHospCap_",abbrev_outcome,"_offset.RData"))
}else{
save(df_fig5_ExceedHospCap,
     file = paste0("figs/data_frames/df_fig5_ExceedHospCap_",abbrev_outcome,".RData"))
}
...
df_figS4_Rt <- rbind.data.frame(df_Rt_BaseCase,
                                df_RTt_INT)
save(df_figS4_Rt,
     file = paste0("figs/data_frames/df_figS4_Rt.RData"))
...
save(df_figS6_DetecInf_prop,
     file = paste0("figs/data_frames/df_figS6_DetecInf_prop.RData"))
...
# Save plot
ggsave(paste0("figs/figs_paper/figS6_DetecInf_prop_SA_SQ.jpg"),
...
outcome_eng <-  "Recovered prevalence proportion"
...
save(df_figS5_Rec_prop,
     file = paste0("figs/data_frames/df_figS5_Rec_prop.RData"))
...
# Save plot
ggsave(paste0("figs/figs_paper/figS5_Rec_prop_SA_SQ.jpg"), 
       width = 18, height = 9, dpi = 300)
...
save(df_figS7_PercDiff,
     file = paste0("figs/data_frames/df_figS7_PercDiff.RData"))
...
# Save plot
ggsave(paste0("figs/figs_paper/figS7_PercDiff_SQ_Holidays_SA_SQ.jpg"),
