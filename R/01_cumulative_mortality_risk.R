...
#' 
#' @param save_data Flag (default is FALSE) of whether to save data as a csv file.
#' @return 
#' A data.frame with cumulative mortality risk expected by 30 and 60 days.
#' @export
get_cum_mort_risk <- function(save_data = FALSE){
  ...
  #which one had the best fit. Here are the results using a Bayesian information 
  ...
  mdl_best <- survival::coxph(survival::Surv(time_dx, dead_ind) ~ survival::pspline(day_dx, 10), 
                              data = cov_ZM_dic)
  ...
  #We create a variable day_dx developing a sequence starting from the day 0 to the 
  #last day of confirmed reported in the official data
  
  df_ND_MR_ZM <- with(cov_ZM_dic, expand.grid(
    day_dx = seq(0, max(day_dx))))
  ...
  df_ND_MR_ZM <- df_ND_MR_ZM %>%
    mutate(date = seq(as.Date("2020-02-24"),as.Date("2020-12-07"),by = 1))
  ...
  survMR_10k_ZM_ps <- survival::survfit(mdl_best, newdata = df_ND_MR_ZM)
...
#' @param n_MR_day Day of expected mortality risk. Options: 30, 60.
